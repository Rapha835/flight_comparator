...
| `/premium` | prix cabines avant (éco premium / affaires / première) à la demande (via Duffel) |
...
- **Le module cabines premium (éco premium / affaires / première) utilise l'API Duffel en direct.** Il faut un jeton Duffel **live** (le mode test renvoie des données fictives). Duffel fixe lui-même la devise de chaque offre. Le module est isolé et n'affecte jamais le suivi principal.
...
`/premium` te dit franchement la cause : « aucune offre sur ces routes/dates » (pas de vol dans cette cabine aux dates testées), ou une erreur `Duffel … HTTP 401/403` si le jeton est invalide ou encore en **mode test** (le mode test ne renvoie que des vols fictifs « Duffel Airways » — il faut passer ton compte en **live**). C'est attendu — le module est volontairement isolé et son échec n'affecte jamais le suivi principal. Désactive-le avec `PREMIUM_ENABLED: false` si tu ne t'en sers pas.
...
Tout tourne dans les quotas gratuits de Google Apps Script ; l'API Data de Travelpayouts (éco) est gratuite, et l'API Duffel (cabines avant) l'est aussi pour la recherche d'offres. Tes tokens restent dans TON projet Apps Script (ne les committe jamais dans un fork public du repo) et rien n'est exposé sur internet : le script sort chercher ses données, personne ne peut l'appeler de l'extérieur. Seuls les messages venant de ton `chat_id` sont pris en compte.

## Stack

Google Apps Script · [Travelpayouts/Aviasales Data API](https://support.travelpayouts.com/hc/en-us/articles/203956163-Aviasales-Data-API) (éco) · [Duffel API](https://duffel.com/docs) (cabines avant) · Google Sheets · Telegram Bot API — entièrement gratuit.
